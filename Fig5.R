...
## Fig 5A-D diversity by pupae present/absent
...
# ASV richness by pupae present/absent, unrarefied
features.pupaepa <- ggplot(data=subset(metadata_div,Pupae_pa != "NA"), 
...
features.pupaepa
kruskal.test(shannon_unrar ~ Pupae_pa, data = metadata_div)
kruskal.test(features_unrar ~ Pupae_pa, data = metadata_div)

## Fig 5B. Shannon by pupae p/a
# shannon diversity by pupae present/absent, unrarefied
shannon.pupaepa <- ggplot(data=subset(metadata_div,Pupae_pa != "NA"), 
...
# relative abundance of Proteobacteria by pupae present/absent
Proteobacteria.pupaepa <- ggplot(data=subset(metadata_div,Pupae_pa != "NA"), 
...
# relative abundance of genus C39 by pupae present/absent
library(ggsignif)
...
metadata.relabund %>% head
write.csv(metadata.relabund, "metadata_C39asvs.csv")
metadata.relabund <- read.csv("metadata_C39asvs.csv")
metadata.relabund
...
metadata.relabund.long <- reshape2::melt(data=metadata.relabund,
                                 ...
                                 variable.name="asvid",
                                 value.name="relabund") 
head(metadata.relabund.long, 25)
# write.csv(metadata.relabund.long,"metadata_relabund_long.csv")
